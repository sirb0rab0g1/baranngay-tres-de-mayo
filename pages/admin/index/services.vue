...
<script>
  ...
  export default {
    ...
    methods: {
      async view (payload) {
        await axios.post('http://20.189.115.250/api/search-barangay', {id: payload.id, barangay: ''}).then(data => {
      ...
      async save () {
        if (_.has(this.form, 'id')) {
          await axios.post('http://20.189.115.250/api/update-barangay', {barangay: this.form.barangay, id: this.form.id}).then(data => {
            this.showbarangay = false
            this.getallbarangay()
            this.form = {}
          })
        } else {
          await axios.post('http://20.189.115.250/api/create-barangay', this.form).then(data => {
            this.showbarangay = false
            this.getallbarangay()
            this.form = {}
          })
      ...
      async searchbarangay () {
        await axios.post('http://20.189.115.250/api/search-barangay', {barangay: _.isNull(this.search) ? '' : this.search, id: null}).then(data => {
          this.requests = data.data
        })
      },
      isNull (param) {
      ...
      async showusersinbarangay (param) {
        await axios.post('http://20.189.115.250/api/get-users-in-barangay', {barangay: param}).then(data => {
          this.showusersinbrgy = true
          this.usersinbrangay = data.data
        })
      },
      async deletebarangay (payload) {
        await axios.post('http://20.189.115.250/api/delete-barangay', {id: payload}).then(data => {
          this.snackbar = true
