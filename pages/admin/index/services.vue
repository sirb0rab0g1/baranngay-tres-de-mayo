...
<script>
  ...
  export default {
    ...
    methods: {
      async view (payload) {
        await axios.post('http://localhost:5000/api/search-barangay', {id: payload.id, barangay: ''}).then(data => {
      ...
      async save () {
        if (_.has(this.form, 'id')) {
          await axios.post('http://localhost:5000/api/update-barangay', {barangay: this.form.barangay, id: this.form.id}).then(data => {
      ...
      async searchbarangay () {
        await axios.post('http://localhost:5000/api/search-barangay', {barangay: _.isNull(this.search) ? '' : this.search, id: null}).then(data => {
          this.requests = data.data
        })
      },
      isNull (param) {
      ...
      async deletebarangay (payload) {
        await axios.post('http://localhost:5000/api/delete-barangay', {id: payload}).then(data => {
          this.snackbar = true
