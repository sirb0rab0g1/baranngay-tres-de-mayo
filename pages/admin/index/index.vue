...
<script>
  ...
  export default {
    ...
    methods: {
      async getallreports () {
        await axios.get('http://localhost:5000/api/get-all-concerns').then(data => {
      ...
      async getallconcernscount () {
        await axios.get('http://localhost:5000/api/get-all-concerns-count').then(data => {
          this.totalconcernscount = data.data.data[0]
          this.getallbarangayscount()
        })
      },
      async getallbarangayscount () {
        await axios.get('http://localhost:5000/api/get-all-barangays-count').then(data => {
          this.totalbarangayscount = data.data.data[0]
        })
      },
      async getalluserscount () {
        await axios.get('http://localhost:5000/api/get-all-users-count').then(data => {
          this.totalusercount = data.data.data[0]
          this.getallconcernscount()
        })
      },
      async searchnow () {
        await axios.post('http://localhost:5000/api/search-admin-contactus', {search: _.isNull(this.search) ? '' : this.search }).then(data => {
          this.requests = data.data
        })
      },
      isNull (param) {
